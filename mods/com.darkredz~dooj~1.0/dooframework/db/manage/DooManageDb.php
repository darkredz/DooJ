...
abstract class DooManageDb
{
    ...
    protected $_tableDefinitionCache = [];
    ...
    public function renameTable($originalTableName, $newTableName)
    {
        ...
        try {
            ...
            throw new DooManageDbException("Error Renaming Table : {$originalTableName} to {$newTableName}",
                $ex->getMessage());
        }
    }
    ...
    protected function buildColumnDefinition($name, $attributes)
    {
        ...
        if (array_key_exists($type, $this->colTypeMapping) == false) {
            throw new DooManageDbException('Unrecognised column type specified',
                "$type is not a recognised database type. Please check DooDbManager::colTypeMapping for supported column types");
        }
        ...
        switch ($type) {
            ...
            case DooManageDb::COL_TYPE_VARCHAR:
                if ($size < 1 || $size > 255) {
                    throw new DooManageDbException("Invalid Column Size ($size) for $name",
                        "char and varchar require a column size between 1 and 255");
                } else {
            ...
            case DooManageDb::COL_TYPE_DECIMAL:
                if ($size < 1 || $size > 255) {
                    throw new DooManageDbException("Invalid Column Size ($size) for $name",
                        "decimal requires size to be between 1 and 255");
                } elseif ($scope < 0 || $scope > $size) {
                    throw new DooManageDbException("Invalid Column Scope ($scope) for $name",
                        "Scope must be between 0 and the Column Size ($size)");
                } else {
    ...
    public function getIndexes($table)
    {
        ...
        foreach ($results as $result) {
            if (!isset($indexes[$result['Key_name']])) {
                $indexes[$result['Key_name']] = ['columns' => [], 'unique' => ($result['Non_unique'] == 0)];
            }
    ...
    protected function checkIdentifier($type, $name)
    {
        ...
        $len = strlen($name);
        if ($len < 1 || $len > 30) {
            throw new DooManageDbException("Invalid Identifer for $type : $name",
                "Identifiers must be between 1 and 30 characters long");
        }
        ...
        if (preg_match($regex, $name) == false) {
            throw new DooManageDbException("Invalid Identifer for $type : $name",
                "Identifer must start with a letter a-z and contain only letters, numbers and underscores");
        }

        // must not have two or more underscores in a row
        if (strpos($name, '__') !== false) {
            throw new DooManageDbException("Invalid Identifer for $type : $name",
                "Identifier can not contain 2 or more underscores consecutavily");
        }
    }
}
