...
class DooSqlMagic
{
    ...
    public function enableSqlTracking()
    {
        $this->sqlTracking = true;
    }

    public function disableSqlTracking()
    {
        $this->sqlTracking = false;
    }

    public function disableDbOperation()
    {
        $this->noDbOperation = true;
    }

    public function enableDbOperation()
    {
        $this->noDbOperation = false;
    }
...
//            return Doo::loadModel($className, $createObj);
     ...
     * @param array $config Associative array of the configurations
     * @param string $useConfigName Default configuration to be used
     */
    public function setDbConfig($dbconfig, $useConfigName)
    {
        $this->configNameUsed = $useConfigName;
        $this->configList = $dbconfig;
        $this->config = $dbconfig[$useConfigName];
    }
    ...
    public function autoconnect($master = false)
    {
        if (!$this->connected) {
            $this->connect($master);
        }
    ...
    public function connect($master = false)
    {
        ...
        try {
            if ($this->config[4] == 'sqlite') {
                $this->pdo = new PDO("{$this->config[4]}:{$this->config[0]}");
            } else {
                if ($this->config[4] == 'oci') {
                    $this->pdo = new PDO("oci:dbname=//{$this->config[0]}/{$this->config[1]}", $this->config[2],
                        $this->config[3], [PDO::ATTR_PERSISTENT => $this->config[5]]);
                } else {
                    $this->pdo = new PDO("{$this->config[4]}:host={$this->config[0]};dbname={$this->config[1]}",
                        $this->config[2], $this->config[3], [PDO::ATTR_PERSISTENT => $this->config[5]]);
                }
            }
            $this->pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $this->connected = true;
            if (isset($this->config['charset']) && isset($this->config['collate'])) {
     ...
     */
    public function reconnect($db_config_name = null, $masterOrSlave = 'master')
    {
        //$host='localhost', $db='', $user='', $password='', $driver='mysql', $persist=true
        if (isset($db_config_name)) {
            $dbconfig = $this->configList[$db_config_name][$masterOrSlave];
            //if it's array of slave should randomly choose one for reconnection
            if (is_array($dbconfig)) {
                $dbconfig = array_rand($dbconfig,1);
            }
        ...
        try {
            if ($dbconfig[4] == 'sqlite') {
                ...
                if ($this->config[4] == 'oci') {
                    ...
                    $this->pdo = new PDO("{$dbconfig[4]}:host={$dbconfig[0]};dbname={$dbconfig[1]}", $dbconfig[2],
                        $dbconfig[3], [PDO::ATTR_PERSISTENT => $dbconfig[5]]);
                }
            }
            $this->pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $this->connected = true;
    ...
    public function exec($query, $param = null)
    {
        $this->autoconnect();
                ...
                if (isset($param[0])) {
                    ...
                    foreach ($querytrack as $k => $v) {
                        ...
                        if (isset($param[$k - 1])) {
                            ...
                            throw new SqlMagicException('Insufficent paramters provided for query', SqlMagicException::PARAMETER_UNMATCH);
                        }
                    }
                    $querytrack = $q;
                } else {
    ...
    */
    public function fetchRow(
        ...
        $param = null,
        $fetchMode = null,
        $fetchModeOptionalParam1 = null,
        $fetchModeOptionalParam2 = []
    )
    {
    ...
    */
    ...
    public function getOne($model, $opt = null)
    {
        ...
        return $this->find($model, ['limit' => 1]);
    }
    ...
    public function find($model, $opt = null)
    {
            ...
            foreach ($opt['filters'] as $filter) {
                ...
                switch ($fmodel_rtype) {
                    ...
                    case 'has_many':
                        ...
                        break;
                    default:
                        throw new SqlMagicException('Table Relationship not defined for ' . $class_name . ' to ' . $fmodel_class, SqlMagicException::UNDEFINED_RELATIONSHIP);
                }
        ...
        if ($opt['limit'] === 1) {
            if (isset($opt['asArray']) && $opt['asArray'] === true) {
                ...
                return $rs->fetchObject($class_name, [$this, $this->app]);
            }
        } else {
            ...
            if (isset($opt['asArray']) && $opt['asArray'] === true) {
                ...
                return $rs->fetchAll(PDO::FETCH_CLASS, $class_name, [$this, $this->app]);
            }
        }
    }
    ...
    public function relate($model, $rmodel, $opt = null)
    {
        ...
        if (is_object($rmodel)) {
            ...
            if ($rtype == null) {
                throw new SqlMagicException("Model $class_name does not relate to $rmodel",
                    SqlMagicException::RELATION_NOT_FOUND);
            }
        ...
        if ($mtype == null) {
            throw new SqlMagicException("Model $rmodel does not relate to $class_name. This is a reverse check.",
                SqlMagicException::RELATION_NOT_FOUND);
        }
        ...
        if (isset($opt['match']) && !($mtype == 'has_many' && $rtype == 'has_many')) {
            if ($opt['match'] == true) {
                self::addWhereNotNull($sqladd['where'], "{$relatedmodel->_table}.{$rparams['foreign_key']}",
                    "{$model->_table}.{$mparams['foreign_key']}");
            } elseif ($opt['match'] == false) {
                self::addWhereIsNull($sqladd['where'], "{$relatedmodel->_table}.{$rparams['foreign_key']}",
                    "{$model->_table}.{$mparams['foreign_key']}");
            }
        }
        ...
        $repeated_vars = array_diff($repeated_vars, ['_table', '_primarykey', '_fields']);
            ...
            if ($r == $relatedmodel->_primarykey) {
                ...
                if (isset($opt['select'])) {
                    if ($opt['select'][0] === '*') {
                        ...
                        if (preg_match('/,(\s+)?' . $relatedmodel->_table . '\.\*/', ',' . $opt['select'])) {
                            $sqladd['select'] .= ", {$relatedmodel->_table}.$r";
                            $opt['select'] .= ", {$relatedmodel->_table}.$r";
                        }
                    }
                ...
                if (isset($opt['select']) && strpos($opt['select'], ',') !== false) {
                    ...
                    if ($opt['select'][0] === '*') {
                        ...
                        if ($modelHasSelectField === false && preg_match('/,(\s+)?' . $model->_table . '\.\*/',
                                ',' . $opt['select'])) {
                            $modelHasSelectField = true;
                        }
                        if ($rmodelHasSelectField === false && preg_match('/,(\s+)?' . $relatedmodel->_table . '\.\*/',
                                ',' . $opt['select'])) {
        ...
        switch ($rtype) {
            ...
            case 'has_many':
                if ($mtype == 'has_many') {
                    ...
                    $model_vars = array_diff($model_vars, ["_{$model->_table}__{$mparams['foreign_key']}"]);
                    $rmodel_vars = array_diff($rmodel_vars, ["_{$relatedmodel->_table}__{$rparams['foreign_key']}"]);
                    ...
                    if ($opt['limit'] != '') {
                        if (!empty ($addonOrder)) {
                            ...
                            $orderLimit = preg_replace("/[, ]*$relatedmodel->_table\.[a-z0-9_-]{1,64}/i", '',
                                $orderLimit);
                        ...
                        if (isset($opt['where']) && $opt['where'] != '') {
                            ...
                            $whrLimit = preg_replace("/[,|AND|OR ]*$relatedmodel->_table\.[a-z0-9_-]{1,64}[^{$model->_table}\.]*/i",
                                '', $opt['where']);
                            if ($whrLimit != $opt['where']) {
                                ...
                                if (preg_match_all("/[,|AND|OR ]*($relatedmodel->_table\.[a-z0-9_-]{1,64}[^{$model->_table}\.]*)/i",
                                        $opt['where'], $rlimitMatch) > 0) {
                                    ...
                                    if (substr($rlimitMatch, strlen($rlimitMatch) - 4) == 'AND ') {
                                        ...
                                        if ($rlimitMatch[strlen($rlimitMatch) - 1] == 'A') {
                                    ...
                                    $mIdStmt = $this->query("SELECT {$mparams['through']}.{$rparams['foreign_key']} FROM {$mparams['through']} WHERE {$rparams['through']}.{$mparams['foreign_key']}=? ORDER BY {$mparams['through']}.{$rparams['foreign_key']} DESC",
                                        [$rStmtId]);
                        ...
                        if (isset($opt['param']) && isset($where_values) && !empty($whrLimit)) {
                            ...
                            $stmtLimit = $this->query("SELECT {$model->_table}.{$model->_primarykey} FROM {$model->_table} WHERE $whrLimit $orderLimit LIMIT {$limitstr}",
                                $varsLimit);
                        } else {
                            if (isset($opt['param']) && !empty($opt['param']) && !empty($whrLimit)) {
                                $stmtLimit = $this->query("SELECT {$model->_table}.{$model->_primarykey} FROM {$model->_table} WHERE $whrLimit $orderLimit LIMIT {$limitstr}",
                                    $opt['param']);
                            } else {
                                if (isset($where_values) && !empty($where_values) && !empty($whrLimit)) {
                                    $stmtLimit = $this->query("SELECT {$model->_table}.{$model->_primarykey} FROM {$model->_table} WHERE $whrLimit $orderLimit LIMIT {$limitstr}",
                                        $where_values);
                                } else {
                            ...
                            foreach ($v as $k2 => $v2) {
                                ...
                                if ($inAsSelect || in_array($k2, $model_vars)) {
                                    if (!$inAsSelect && $oriSel[0] !== '*' &&
                                        ...
                                        !preg_match('/,(\s+)?' . $model->_table . '\.\*/',
                                            ',' . str_replace($model->_table . '.' . str_replace('_' . $model->_table . '__',
                                                    '', $k2), $model->_table . '.*', $oriSel))) {
                                ...
                                if ($gotoRelateSect === true) {
                                    ...
                                    if ($oriSel[0] !== '*') {
                                        if (in_array($k2, $rmodel_vars) === false &&
                                            !preg_match('/,(\s+)?' . $relatedmodel->_table . '\.\*/',
                                                ',' . str_replace($relatedmodel->_table . '.' . $k2,
                                                    $relatedmodel->_table . '.*', $oriSel))) {
                                            continue;
                                        } else {
                                            if (!preg_match("/,(\s+)?({$relatedmodel->_table}\.)?($k2|\*)(\s+)?,/",
                                                ',' . $oriSel . ',')) {
                                                continue;
                                            } else {
                                                if (isset($alias_vars[$k2])) {
                            ...
                            if (!in_array($fk, $model_pk_arr)) {
                                ...
                                foreach ($v as $k2 => $v2) {
                                    ...
                                    if ($inAsSelect || in_array($k2, $model_vars)) {
                                        ...
                                        if (!$inAsSelect && $oriSel[0] !== '*' &&
                                            !preg_match("/,(\s+)?({$model->_table}\.)?$k2(\s+)?,/",
                                                ',' . $oriSel . ',') &&
                                            !preg_match('/,(\s+)?' . $model->_table . '\.\*/',
                                                ',' . str_replace($model->_table . '.' . str_replace('_' . $model->_table . '__',
                                                        '', $k2), $model->_table . '.*', $oriSel))) {
                                        ...
                                        if ($rfk != null) {
                                            if ($k2 === $rretrieved_pk_key) {
                                                ...
                                                if (isset($ralias_vars[$k2])) {
                                                    ...
                                                    if (in_array($k2, $assoc_model->_fields)) {
                                                        $assoc_model->{$k2} = $v2;
                                                    }
                                                }
                                            }
...
//														echo "<h3>$k2 = $v2</h3>";
    ...
    public function relateExpand($model, $rmodel, $opt = null)
    {
        ...
        if (isset($rOpt['where'])) {
            $rOpt['where'] = $newrm->_table . '.' . $newrm->_primarykey . ' IN (' . implode(',',
                    $id) . ') AND ( ' . $rOpt['where'] . ' )';
        } else {
    ...
    public function checkIsDbType($type)
    {
        return ((!empty($this->config[4]) && $this->config[4] == $type) || (!empty($this->config['master'][4]) && $this->config['master'][4] == $type));
    }
...
//                $model->{$rclass_name} = array();
            ...
            if ($rtype == null) {
                throw new SqlMagicException("Model $class_name does not relate to $rclass_name",
                    SqlMagicException::RELATION_NOT_FOUND);
            }
            #print_r(array($rtype,$rparams));

            $chk_rmodel = $this->find($rclass_name, [
                'select' => $rmodel->_primarykey,
                'limit' => 1,
                'where' => $rmodel->_primarykey . " = ?",
                'param' => [$rmodel->{$rmodel->_primarykey}],
            ]);
...
//				$fieldstr = "{$rmodel->_primarykey}$fieldstr";
                ...
                $this->query("INSERT INTO {$rparams['through']} ({$model_linked_key},{$rparams['foreign_key']})  VALUES (?,?)",
                    [$mId, $rId]);
            } else {
                if (isset($rparams['foreign_key'])) {
...
//				$fieldstr = "{$rmodel->_primarykey}$fieldstr";
                ...
                if ($this->query("SELECT {$rparams['foreign_key']},{$model_linked_key} FROM {$rparams['through']} WHERE {$rparams['foreign_key']}=? AND {$model_linked_key}=?",
                        [$rId, $mId])->fetch() == null) {
                    $this->query("INSERT INTO {$rparams['through']} ({$model_linked_key},{$rparams['foreign_key']}) VALUES (?,?)",
                        [$mId, $rId]);
                }
        ...
        if ($updateRelated && count($rIdsToNotDelete) > 0) { // if (isset($opt["deleteNotRelated"]) && $opt["deleteNotRelated"] == true && count($rIdsToNotDelete) > 0) {
            if ($rtype == 'has_many' && isset($rparams['foreign_key']) && isset($rparams['through'])) { // delete into the 'through' Table if needed
                $this->query("DELETE FROM {$rparams['through']} WHERE {$rparams['foreign_key']} NOT IN (" . implode(',',
                        $rIdsToNotDelete) . ") AND {$model_linked_key}=?", [$mId]);
            } else {
                if (isset($rparams['foreign_key']) && count($rIdsToNotDelete) > 0) {
                    $this->query("DELETE FROM {$rmodel->_table} WHERE {$rmodel->_primarykey} NOT IN (" . implode(',',
                            $rIdsToNotDelete) . ") AND {$rparams['foreign_key']}=?", [$mId]);
                }
            }
        }
    }
    ...
    {
        $chk_model = $this->find(get_class($model), [
            'select' => $model->_primarykey,
            'limit' => 1,
            'where' => $model->_primarykey . " = ?",
            'param' => [$model->{$model->_primarykey}],
        ]);
        if ($chk_model) {
...
{
    const UNEXPECTED_CLASS = 0;
    const RELATION_NOT_FOUND = 1;
    const DBCONFIG_NOT_FOUND = 2;
    const DB_CONN_ERROR = 3;
    const PARAMETER_UNMATCH = 4;
    const UNDEFINED_RELATIONSHIP = 5;
}
