...
class DooAcl
{
     ...
     * @var string|array
     */
    public $defaultFailedRoute = ['/error-default/failed-route/please-set-in-route', 404];
    ...
    public function process($role, $resource, $action = '')
    {
        if ($this->isDenied($role, $resource, $action)) {
            ...
            if (isset($this->rules[$role]['failRoute'])) {
                ...
                if (is_string($route)) {
                    ...
                    if (isset($route[$resource])) {
                        ...
                        return (is_string($route['_default'])) ? [$route['_default'], 'internal'] : $route['_default'];
                    }
                }
            }
            ...
            if ($this->isAllowed($role, $resource, $action) == false) {
    ...
    public function setDefaultFailedRoute($route)
    {
        $this->defaultFailedRoute = $route;
    }
}
