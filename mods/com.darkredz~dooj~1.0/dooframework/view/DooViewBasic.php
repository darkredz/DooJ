...
class DooViewBasic
{
    ...
    protected function compileLayout($layoutFilePath, $compiledFilePath)
    {
        ...
        $viewContent = preg_replace_callback('/<!-- placeholder:([a-zA-Z0-9\_\-]+?) -->([\s\S]*?)<!-- endplaceholder -->/',
    ...
    protected function compileTags($str)
    {
        ...
        $str = preg_replace_callback('/' . $this->tagVariable_start . '[ ]*?([^\t\r\n]+?)[ ]*?' . $this->tagVariable_end . '/',
            [&$this, 'convertOutputVariable'], $str);

        // Find and replace blocks in the form of {% blockHanlerFunction DATA %}
        $str = preg_replace_callback('/' . $this->tagBlock_start . '[ ]*([a-zA-Z0-9\-\_]+)([\s\S]*?)[ ]*' . $this->tagBlock_end . '/',
            [&$this, 'convertBlock'], $str);
    ...
    protected function block_for($params)
    {
        ...
        if (preg_match('/([a-z0-9\-_]+) from ([^\t\r\n]+) to ([^\t\r\n]+?) step ([^\t\r\n]+?)( with meta)?$/i', $params,
            ...
            if (preg_match('/([^\t\r\n ]+) as ([a-zA-Z0-9\-_]+)[ ]?=>[ ]?([a-zA-Z0-9\-_]+)( with meta)?/', $params,
                $matches)) {
    ...
    public function compileSubView($file)
    {
        ...
        if (!file_exists($vfilename)) {
            return [false, "<span style=\"color:#ff0000\">View file <strong>{$file}.html</strong> not found</span>"];
        } else {
    ...
    protected function strToStmt($str)
    {
                        ...
                        if ($inFilter) {
                            ...
                            for ($j = 1; $j < count($tokens); $j++) {
                                ...
                                if (($functionStartPos = strpos($token, '(')) !== false) {
                                    $filterResult = $this->filterFunctionPrefix . substr($token, 0,
                                            $functionStartPos) . '(' . $filterResult . ', ' . substr($token,
                                            $functionStartPos + 1);
                                } else {
        ...
        if ($inFilter) {
            ...
            for ($j = 1; $j < count($tokens); $j++) {
                ...
                if (($functionStartPos = strpos($token, '(')) !== false) {
                    $filterResult = $this->filterFunctionPrefix . substr($token, 0,
                            $functionStartPos) . '(' . $filterResult . ',' . substr($token, $functionStartPos + 1);
                } else {
                    $filterResult = $this->filterFunctionPrefix . $token . '(' . $filterResult . ')';
                }
            }
