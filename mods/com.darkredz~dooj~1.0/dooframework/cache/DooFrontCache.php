    ...
    public function __construct($conf, $path = '')
    {
        ...
        if ($path == '') {
            if (isset($this->conf->CACHE_PATH)) {
                $this->_directory = $this->conf->CACHE_PATH . 'frontend/';
            } else {
                $this->_directory = $this->conf->SITE_PATH . $this->conf->PROTECTED_FOLDER . 'cache/frontend/';
            }
        } else {
    ...
    public function flush($url, $recursive = false)
    {
        ...
        if ($recursive) {
            ...
            while (($file = readdir($handle)) !== false) {
                if (is_file($this->_directory . $file) && (strpos($file, $oriUrl1) === 0 || strpos($file,
                            $oriUrl2) === 0)) {
