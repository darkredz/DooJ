...
class DooUriRouter
{
    ...
    public function execute($routeArr, $subfolder = '/')
    {
        ...
        if ($route[0] === 'redirect') {
            if (sizeof($route) === 2) {
                return ['redirect' => [$route[1], 302]];
            } else {
                return ['redirect' => [$route[1], $route[2]]];
            }
        }
        ...
        if (isset($route['className']) === true) {
            return [$route[0], $route[1], $params, $route['className']];
        }

        //return Controller class, method, parameters of the route
        return [$route[0], $route[1], $params];
    }
...
//    }
    ...
    private function connect($routes, $subfolder)
    {
        ...
        if ($skipNormalRoutes === false) {
            ...
            if (isset($possibleRoutes[$requestedUri]) === true) {
                ...
                if (false === strpos($requestedUri, ':')) {
                    ...
                    return [$possibleRoutes[$requestedUri], null];
                }
            }
        }
            ...
            foreach ($possibleRoutes as $routeKey => $routeData) {
                ...
                if (isset($routeData['extension']) === true) {
                    if ($uriExtension !== false || (isset($routeData['extensionOptional']) && $routeData['extensionOptional'] === true)) {
                        ...
                        } elseif (is_array($routeExtension) === true && in_array($uriExtension,
                ...
                foreach ($routeParts as $i => $routePart) {
                    ...
                    if ($routePart[0] === ':') {
                        continue;
                    }    // This routePart is a parameter in the Uri

                    if ($routePart !== $uriParts[$i]) {
                        continue 2;
                    } // The static part of this route does not match the route part
                }

                //$this->log('Got a route match. RouteKey: ' . $routeKey);

                $params = $this->parseParams($uriParts, $routeParts);
                    ...
                    foreach ($routeData['match'] as $paramName => $pattern) {
                        if (is_string($pattern)) {
                            ...
                            if (preg_match($pattern[0], $params[$paramName], $paramMatches) == 0) {
                                ...
                                if (isset($paramMatches[$pattern[1]])) {
        ...
        if (isset($routes['*']['catchall']) === true) {
            ...
            foreach ($catchRoutes as $routeKey => $routeData) {
                ...
                $params = $this->parseParamsCatch($uriParts, $routeParts);
     ...
     * @return array returns an array consist of the Controller class, action method and parameters of the route
     */
    public function autoConnect($subfolder = '/', $autoroute_alias = null)
    {
        ...
        if (isset($this->app->conf->MODULES) === true) {
            if (in_array($uri[0], $this->app->conf->MODULES) === true) {
                ...
                if (empty($this->app->route['autoroute_force_dash']) === false) {
                    $moduleRemUnderscore = explode("\t",
                        str_replace('_', '-', implode("\t", $this->app->conf->MODULES)));
        ...
        $this->app->conf->AUTO_VIEW_RENDER_PATH = [$controller_name];
        ...
        if ($autoroute_alias !== null) {
            ...
            if (isset($autoroute_alias[$alias]) === true) {
                ...
                $this->app->conf->AUTO_VIEW_RENDER_PATH[0] = strtolower(preg_replace('/([A-Z])/', '-$1',
                    substr($convertname, 0, -10)));
            } else {
        ...
        return [$controller_name, $method_name, $method_name_ori, $params, $module];
    }
     ...
     */
    protected function parseParams($req_route, $defined_route)
    {
        $params = [];
     ...
     * @return array An array of parameters found in the requested URI
     */
    protected function parseParamsCatch($req_route, $defined_route)
    {
        $params = [];
        $size = sizeof($req_route);
        for ($i = 0; $i < $size; $i++) {
