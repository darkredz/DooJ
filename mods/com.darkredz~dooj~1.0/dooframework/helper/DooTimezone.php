...
class DooTimezone
{
    ...
    public static function getTimezoneWithOffset($timeformat = 'H:i', $groupby = null)
    {
        ...
        if ($groupby == 'offset') {
            ...
            if ($groupby == 'timezone') {
                ...
                for ($i = 0; $i < $size; $i++) {
                    $tlist[$i] = [self::getOffsetFromTimezone($list[$i], $timeformat), $list[$i]];
                }
            }
        }
     ...
     * @return string
     */
    public static function getCommonTimezoneHTMLList(
        $selectedOffset = null,
        $prefix = '[UTC ',
        $suffix = '] ',
        $timeformat = 'H:i',
        ...
        foreach ($list as $offset => $tz) {
            ...
            if ($offset == 0) {
                ...
                if ($optionFormat == 'H:i') {
                    ...
                    if ($optionFormat == 'hour') {
                        $offset = $offset / 3600;
                    } else {
                        if ($optionFormat == 'minute') {
                            $offset = $offset / 60;
                        }
                    }
                }
     ...
     * @return string
     */
    public static function getTimezoneHTMLList(
        $selectedTimezone = '',
        $useOffsetAsValue = true,
        $prefix = '[UTC ',
        $suffix = '] ',
        $timeformat = 'H:i',
        $optionFormat = 'hour'
