...
class DooTranslator
{
     ...
     * Supported adapters
     */
    private $_supportedAdapters = ["csv", "gettext", "array", "ini"];

    /**
     * Supported cache types
     */
    private $_supportedCacheTypes = ['apc', 'php', 'xcache', 'eaccelerator', 'file'];
     ...
     */
    public function __construct($adapter, $data, $options = [])
    {
        $adapter = strtolower($adapter);
        if (!in_array($adapter, $this->_supportedAdapters)) {
            throw new DooTranslatorException($adapter . " is not supported by DooTranslator, supported types are: " . implode(', ',
                    $this->_supportedAdapters));
        }
        ...
        if (isset($this->_options['cache'])) {
            if (!in_array($this->_options['cache'], $this->_supportedCacheTypes)) {
                throw new DooTranslatorException($this->_options['cache'] . " is unsuported cache type, supported cache types are: " . implode(', ',
                        $this->_supportedCacheTypes));
            } else {
     ...
     * </code>
     */
    public function translate($messageId, $params = [])
    {
        if ($this->_adapter == null) {
            throw new DooTranslatorException("Adapter not set!");
        }
        $translatedMessage = "";
    ...
    private function _loadData($data)
    {
        ...
        switch ($this->_adapter) {
            ...
            case 'csv':
                ...
                while (($data = fgetcsv($this->_file, $options['length'], $options['delimiter'],
                        $options['enclosure'])) !== false) {
